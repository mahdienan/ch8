...
class CH8DA:
		...
		for code in self.data:
			...
			if not self.end_of_code: # loop until endless jump
				if hexcode & 0xFFFF == 0x00E0:
					...
					self.disassemble_0x0XXX(hexcode)	
	...
	# SYS addr (0snnn)
	def disassemble_0x0XXX(self, hexcode):
		print "GOTO 0x{:03X}".format(hexcode & 0x0FFF)

	# CLR
	def disassemble_0x00E0(self, hexcode):
		print "CLS"

	# RET
	def disassemble_0x00EE(self, hexcode):
		print "RET"

	# JP addr (1nnn)
	def disassemble_0x1XXX(self, hexcode, location):
	...
	# CALL addr (2nnn)
	def disassemble_0x2XXX(self, hexcode):
		print "CALL " + "#{:03X}".format(hexcode & 0x0FFF)

	# SE Vx, byte (3xkk)
	def disassemble_0x3XXX(self, hexcode):
		print "SE " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", #{:02X}".format(hexcode & 0x00FF)

	# SNE Vx, byte (4xkk)
	def disassemble_0x4XXX(self, hexcode):
		print "SNE " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", #{:02X}".format(hexcode & 0x00FF)

	# SE Vx, Vy (5xy0)
	def disassemble_0x5XXX(self, hexcode):
		print "SE " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2]

	# LD Vx, byte (6xkk)
	def disassemble_0x6XXX(self, hexcode):
		print "LD " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", #{:02X}".format(hexcode & 0x00FF)

	# ADD Vx, byte (7xkk)
	def disassemble_0x7XXX(self, hexcode):
		print "ADD " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", #{:02X}".format(hexcode & 0x00FF)

	# LD Vx, Vy (8xy0)
	def disassemble_0x8XX0(self, hexcode):
		print "LD " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2]

	# OR Vx, Vy (8xy1)
	def disassemble_0x8XX1(self, hexcode):
		print "OR " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2]

	# AND Vx, Vy (8xy2)
	def disassemble_0x8XX2(self, hexcode):
		print "AND " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2]

	# XOR Vx, Vy (8xy3)
	def disassemble_0x8XX3(self, hexcode):
		print "XOR " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2]

	# ADD Vx, Vy (8xy4)
	def disassemble_0x8XX4(self, hexcode):
		print "ADD " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2][:1]

	# SUB Vx, Vy (8xy5)
	def disassemble_0x8XX5(self, hexcode):
		print "SUB " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2][:1]

	# SHR Vx {, Vy} (8xy6)
	def disassemble_0x8XX6(self, hexcode):
		print "SHR " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", #{:02X}".format(hexcode & 0x00FF)

	# SUBN Vx, Vy (8xy7)
	def disassemble_0x8XX7(self, hexcode):
		print "SUBN " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2][:1]

	# SHL Vx {, Vy} (8xyE)
	def disassemble_0x8XXE(self, hexcode):
		print "SHL " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2][:1]

	# SNE Vx, Vy (9xy0)
	def disassemble_0x9XX0(self, hexcode):
		print "SNE " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v{:01X}".format(hexcode & 0x00F0)[:2][:1]

	# LD I, addr (Annn)
	def disassemble_0xAXXX(self, hexcode):
		print "LD I, " + "#{:03X}".format(hexcode & 0x0FFF)

	# JP V0, addr (Bnnn)
	def disassemble_0xBXXX(self, hexcode):
		print "JP V0, " + "#{:03X}".format(hexcode & 0x0FFF)

	# RND Vx, byte (Cxkk)
	def disassemble_0xCXXX(self, hexcode):
		print "RND " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", #{:02X}".format(hexcode & 0x00FF)

	# DRW Vx, Vy, nibble (Dxyn)
	def disassemble_0xDXXX(self, hexcode):
		print "DRW " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", v" + "{:01X}".format(hexcode & 0x00F0)[0:1] + ", #" + "{:01X}".format(hexcode & 0x000F)
	# SKP Vx (Ex9E)
	def disassemble_0xEX9E(self, hexcode):
		print "SKP " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# SKNP Vx (ExA1)
	def disassemble_0xEXA1(self, hexcode):
		print "SKNP " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# LD Vx, DT (Fx07)
	def disassemble_0xFX07(self, hexcode):
		print "LD " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", DT"
	# LD Vx, K (Fx0A)
	def disassemble_0xFX0A(self, hexcode):
		print "LD " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1] + ", K"

	# LD DT, Vx (Fx15)
	def disassemble_0xFX15(self, hexcode):
		print "LD DT, " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# LD ST, Vx (Fx18)
	def disassemble_0xFX18(self, hexcode):
		print "LD ST, " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# ADD I, Vx (Fx1E)
	def disassemble_0xFX1E(self, hexcode):
		print "ADD I, " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# LD F, Vx (Fx29)
	def disassemble_0xFX29(self, hexcode):
		print "LD F, " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# LD B, Vx (Fx33)
	def disassemble_0xFX33(self, hexcode):
		print "LD B, " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# LD [I], Vx (Fx55)
	def disassemble_0xFX55(self, hexcode):
		print "LD [I], " + "v" + "{:01X}".format(hexcode & 0x0F00)[:1]

	# LD Vx, [I] (Fx65)
	def disassemble_0xFX65(self, hexcode):
